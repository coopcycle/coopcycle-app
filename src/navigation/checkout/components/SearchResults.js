...
export default (props) => {
  ...
  const query = 'Albanaise'

  const topResults = SearchEngine.search(`=${query}`)
  const topIndex = _.map(topResults, 'refIndex')
  const altResults = SearchEngine.search(query)
  ...
  return <View style={{ flex: 1 }}>
    <View style={{ flex: 1, paddingTop: 60 }}>
      <View style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: 60 }}>
        <GroupImageHeader image="https://i.imgur.com/PG8SO0n.jpeg" text={query} />
      </View>
      <View padding={5}>
