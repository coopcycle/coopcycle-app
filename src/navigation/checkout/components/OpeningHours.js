...
const OpeningHours = (props) => {
  ...
  const weekdays = props.openingHoursSpecification.state.reduce((acc, day, index) => {
    const hours = day.ranges.reduce((acc2, range, _index) => {
      acc2.push(<Text textAlign={'center'} key={`r${_index}`} paddingX={3} >{range.join(' - ')}</Text>)
      return acc2
    }, [])
    acc.push(<HStack paddingY={1} key={`h${index}`} backgroundColor={day.today ? highlight : background}>
