...
class OrderDelayScreen extends Component {
  _delayOrder(delay) {
    this.props.delayOrder(this.props.route.params?.order, delay, order =>
    ...
    return (
      <Center flex={1}>
        <View style={{ padding: 20 }}>
          <Text note>{this.props.t('RESTAURANT_ORDER_DELAY_DISCLAIMER')}</Text>
        </View>
        <View style={{ flex: 1 }}>
          <BigButton
            heading={'10 minutes'}
...
export default connect(
  () => ({}),
  mapDispatchToProps,
)(withTranslation()(OrderDelayScreen));
