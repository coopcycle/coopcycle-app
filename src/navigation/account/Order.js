    ...
    if (order) {
      return <>
        <VStack flex={ 1 }>
          ...
          {(order.state === 'fulfilled' && order.hasReceipt) && <Button onPress={() => this.props.shareInvoice(order)}
            leftIcon={<Icon as={Ionicons} name="share-outline" size="sm" />}
            margin={5} variant="subtle" colorScheme="secondary">
            {i18n.t('SHARE_INVOICE')}
          </Button>}
          {(order.state === 'fulfilled' && !order.hasReceipt) && <Button onPress={() => {
            this.props.showAddressModal(i18n.t('BILLING_ADDRESS'))
          }}
                                                                      leftIcon={<Icon as={Ionicons} name="receipt-outline" size="sm" />}
                                                                      margin={5} variant="subtle" colorScheme="secondary" isLoading={this.props.loading}>
            {i18n.t('GENERATE_INVOICE')}
          </Button>
          }
        </VStack>
        {(order.state === 'fulfilled' && !order.hasReceipt) && <AddressModal value={order?.shippingAddress} onSelect={(address) => {
