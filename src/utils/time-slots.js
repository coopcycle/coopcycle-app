...
const hasOpeningHours = timeSlot =>
  timeSlot.hasOwnProperty('openingHoursSpecification') &&
  Array.isArray(timeSlot.openingHoursSpecification) &&
  timeSlot.openingHoursSpecification.length > 0;

const getCursor = (timeSlot, now) => {
  if (
    !hasOpeningHours(timeSlot) &&
    timeSlot.workingDaysOnly &&
...
export function getChoicesWithDates(timeSlot, now) {
  ...
  const choices = Array.isArray(timeSlot.choices)
    ? timeSlot.choices
    : _.values(timeSlot.choices);
  let items = [];
  ...
  while (countNumberOfDays(items) < expectedNumberOfDays) {
    ...
    cursor = timeSlot.workingDaysOnly
      ? cursor.nextBusinessDay()
      : cursor.add(1, 'day');
  }
  ...
  return items.map(item => ({
    key: `${moment(item.start).format('YYYY-MM-DD')} ${item.start.format(
      'HH:mm',
...
export function humanizeTaskTime(task, now) {
  ...
  const range = moment.range(moment(task.after), moment(task.before));

  return makeLabel(range, now);
}

export function isCartTimingValid({
  cart,
  ...
  if (!openingHoursSpecification.isOpen(shippedAt)) {
    console.log(
      '[isCartTimingValid]: shippedAt is defined on a closed schedule',
    );
    return false;
  }
