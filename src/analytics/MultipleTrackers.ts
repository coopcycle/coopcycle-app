...
MultipleTrackers.prototype.setCurrentScreen = function (
  screenName,
  screenClassOverride,
) {
  this.trackers.forEach(tracker =>
    tracker.setCurrentScreen(screenName, screenClassOverride),
  );
};

MultipleTrackers.prototype.logEvent = function (
  category,
  action,
  text,
  number,
  ...
  this.trackers.forEach(tracker =>
    tracker.logEvent(category, action, text, number),
  );
};
