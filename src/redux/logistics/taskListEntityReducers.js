...
export default (state = initialState, action) => {
  switch (action.type) {
    ...
    case CREATE_TASK_SUCCESS: {
      ...
      if (task.isAssigned) {
        let newItems = taskListEntityUtils.addAssignedTask(selectors.selectEntities(state), task)
    ...
    case ASSIGN_TASK_SUCCESS: {
      let newItems = taskListEntityUtils.addAssignedTask(selectors.selectEntities(state), action.payload)
      return taskListAdapter.upsertMany(state, newItems)
    }
    case UNASSIGN_TASK_SUCCESS: {
      let newItems = taskListEntityUtils.removeUnassignedTask(selectors.selectEntities(state), action.payload)
      return taskListAdapter.upsertMany(state, newItems)
    }
    default:
      return state
