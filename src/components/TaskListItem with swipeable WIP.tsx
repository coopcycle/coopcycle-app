...
const TaskListItem = forwardRef<SwipeableRef, TaskListItemProps>(
  (
    {
      task,
      nextTask,
    ...
    const styleAnimationLeft = useAnimatedStyle(() => {
      return {
        //transform: [{ translateX: drag.value - visibleButtonWidth }],
        transform: [{ translateX: buttonWidth - visibleButtonWidth }],
      };
    });
    const styleAnimationRight = useAnimatedStyle(() => {
      return {
        //transform: [{ translateX: drag.value + visibleButtonWidth }],
        transform: [{ translateX: buttonWidth + visibleButtonWidth }],
      };
    ...
    const renderLeftActions = (prog, drag) => {
      return (
        <Reanimated.View style={styleAnimationLeft}>
          {/* <Pressable
    ...
    const renderRightActions = (prog, drag) => {
      return (
        <Reanimated.View style={styleAnimationRight}>
          {/* <Pressable
            className="items-center justify-center"
            style={{ flex: 1, width: visibleButtonWidth, backgroundColor: yellowColor }}
    ...
    //   }
    // }
    function _onRowOpen(direction: string) {
      if (direction === 'left' && onSwipedToLeft) {
        onSwipedToLeft();
      } else if (direction === 'right' && onSwipedToRight) {
        onSwipedToRight();
      }
    }
